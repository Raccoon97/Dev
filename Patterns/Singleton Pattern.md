...
# Singleton Pattern 을 사용하면 안되는 이유
- **테스트가 어렵다** : 전역 상태를 공유하므로 단위 테스트 시 독립적인 환경을 만들기 어렵고, Mock 객체로 교체하기 힘들다.
- **의존성을 숨긴다** : 클래스가 Singleton 에 직접 접근하면 외부에서 해당 의존성을 파악하기 어렵다. 생성자 주입 방식이 더 명시적이다.
- **전역 상태 문제** : 어디서든 접근 가능하므로 상태 변화를 추적하기 어렵고, 멀티스레드 환경에서 동기화 이슈가 발생할 수 있다.
- **개방-폐쇄 원칙 (OCP) 위반** : Singleton 클래스에 직접 의존하면 다른 구현으로 교체하기 어려워 확장에 닫혀 있게 된다.
- **생명주기 관리가 어렵다** : 앱이 종료될 때까지 메모리에 남아있어 불필요한 자원을 소비할 수 있다.

> Singleton 자체가 나쁜 것이 아니라, **남용** 이 문제다. 꼭 필요한 경우에만 사용하고 가능하면 **의존성 주입 (DI)** 으로 대체하는 것이 좋다.
